...
const INPUT_COST_PER_TOKEN = 0.00000035;
const OUTPUT_COST_PER_TOKEN = 0.00000105;
...
export class GeminiProvider implements IAIProvider {
  ...
  async initialize(config: Record<string, string>): Promise<void> {
    ...
    if (key === undefined || key.length === 0) {
      throw new ConfigurationError('GeminiProvider requires GEMINI_API_KEY in config');
    }
    this.apiKey = key;
    this.available = true;
  ...
  async execute(_request: AIRequest): Promise<AIResponse> {
    throw new AIProviderNotImplementedError(ProviderType.Gemini);
  }
  ...
  async shutdown(): Promise<void> {
    ...
    this.apiKey = null;
    this.logger.info('GeminiProvider shut down');
  }
